...
if [ ! -f templates/config.ini.template ]
then
    echo "templates/config.ini.template does not exist, exiting"
    exit 1
fi

clear

echo -n "Moloch service userid: [moloch] "
read USERNAME
if [ -z $USERNAME ]; then USERNAME="moloch"; fi

echo -n "Moloch service groupid: [moloch] "
read GROUPNAME
if [ -z $GROUPNAME ]; then GROUPNAME="moloch"; fi
...
sed -e "s/_PASSWORD_/${PASSWORD}/g" -e "s/_USERNAME_/${USERNAME}/g" -e "s/_GROUPNAME_/${GROUPNAME}/g" -e "s/_INTERFACE_/${INTERFACE}/g"  -e "s,_TDIR_,${TDIR},g" \
    templates/config.ini.template | sudo tee ${TDIR}/etc/config.ini > /dev/null

SUBJECT="/C=${COUNTRY}/ST=${STATE}/L=${LOCALITY}/O=${ORG_NAME}/OU=${ORG_UNIT}/CN=${FQDN}/emailAddress=moloch@localhost/"

cd ${TDIR}/etc/
sudo openssl req -new -newkey rsa:2048 -nodes -keyout moloch.key -out moloch.csr -subj $SUBJECT 
sudo openssl x509 -req -days 3650 -in moloch.csr -signkey moloch.key -out moloch.crt
